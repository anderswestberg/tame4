export class TAME {
    ...
    dateNames: { weekdShort: any; weekdLong: any; monthsShort: any; monthsLong: any }
    maxStringLen: number
    maxDropReq: number
    ...
    getItemInformation(item) {
        var itemInfo = {} as any, arrPlcVarName, splitType;
    ...
    async adsReqSendAsync(adsReq) {
        ...
        let response = await fetch(this.service.serviceUrl, options);
        let str = await response.text()
        const xmlDocument = new window.DOMParser().parseFromString(str, "text/xml") as XMLDocument
        return this.parseResponse(adsReq, xmlDocument)
    }
    ...
    createRequest(adsReq) {
        ...
        adsReq.send = async () => await this.adsReqSendAsync(adsReq)
        return adsReq;
    }
     ...
     * @param {Number} max
     */
    checkValue(item: { val: string }, type: string, min: string | number, max: string | number) {
        var val;
    ...
    createStructDef(structname) {
        ...
        for (subitem in subitems) {
            if (subitems[subitem].type === "STRUCT") { // !!!!!!!!!!!!!!!!!!! Was USER ?????????????????????
    ...
    parsePlcLreal(string) {
        var num = this.parsePlcUdint(string.substring(4, 8)),
            num2 = this.parsePlcUdint(string.substring(0, 4)),
            i = 12,
            mant = 1,
            dual = 0.5,
            sign, exp;
     ...
     */
    parseReadReq(adsReq, response) {

        let itemList = adsReq.reqDescr.items,
            arrType = [],
            strAddr = 0,
     ...
     */
    parseSumReadReq(adsReq, response) {
     ...
     * @param {Object} adsReq   ADS Request Object
     */
    parseSumWriteReq(adsReq, response) {
     ...
     * @param {Object} adsReq   ADS Request Object
     */
    parseHandles(adsReq, response) {

        var response,
            arrSymNames = this.handleNames,
            strAddr = 0,
            subStrAddr = 0,
    ...
    async writeAccessStruct(method, args) {
        let reqDescr = this.createStructDescriptor(method, args)
        let adsReq = this.writeReq(reqDescr)
        this.createRequest(adsReq)
        let value = await this.adsReqSendAsync(adsReq)
        return value
    ...
    async readAccessStruct(method, args) {
        let reqDescr = this.createStructDescriptor(method, args)
        let adsReq = this.readReq(reqDescr)
        this.createRequest(adsReq)
        let value = await this.adsReqSendAsync(adsReq)
    ...
    createStructDescriptor(method, args) {
        ...
        var reqDescr = {} as any,      //Request Descriptor
    ...
    writeReq(reqDescr) {
        ...
        for (idx = 0, listlen = itemList.length; idx < listlen; idx++) {
            ...
            if (isNaN(this.plcTypeLen[type]) && type != "STRING" && type != "WSTRING") {
            ...
            if (reqDescr.calcAlignment === true && len > 1 && type !== 'STRING' && type !== 'WSTRING' && type !== 'EndStruct' && pData.length > 0) {
        ...
        if (typeof reqDescr.readLength !== 'number') {
            ...
            for (idx = 0, listlen = itemList.length; idx < listlen; idx++) {
                ...
                if (reqDescr.seq === true) {
                    ...
                    if (reqDescr.calcAlignment === true && vlen > 1 && type !== 'EndStruct' && type !== 'STRING' && type !== 'WSTRING' && reqDescr.readLength > 0) {
     ...
     * @param {Object} adsReq   The object containing the arguments of the ADS request.
     */
    parseResponse(adsReq, xmlDocument: XMLDocument) {
        ...
        try {
            //Get errors
            let faultTag = response.getElementsByTagName('faultstring')
            if (faultTag.length) {
        ...
        if (!errorText) {
            ...
            if (adsReq.method === 'ReadState') {
                ...
                this.parseAdsState(adsReq, response);

            } else if (adsReq.method === 'Read' || adsReq.method === 'ReadWrite') {

                switch (adsReq.indexGroup) {
                    case this.indexGroups.UploadInfo:
                        this.parseUploadInfo(adsReq, response);
                        break;
                    case this.indexGroups.Upload:
                        this.parseUpload(adsReq, response);
                        break;
                    case this.indexGroups.SumRd:
                        this.parseSumReadReq(adsReq, response);
                        break;
                    case this.indexGroups.SumWr:
                        this.parseSumWriteReq(adsReq, response);
                        break;
                    case this.indexGroups.SumRdWr:
                        this.parseHandles(adsReq, response);
                        break;
                    default:
                        result = this.parseReadReq(adsReq, response);
                }
            }
            ...
            this.log('TAME library error: Message from server:  ' + errorText + " code: " + errorCode);
        return result
    };
     ...
     * @param {Object} args
     */
    writeBool = async (args) => await this.writeSingle('Write', 'BOOL', args)
    writeByte = async (args) => await this.writeSingle('Write', 'BYTE', args)
    writeUsint = async (args) => await this.writeSingle('Write', 'USINT', args)
    writeSint = async (args) => await this.writeSingle('Write', 'SINT', args)
    writeWord = async (args) => await this.writeSingle('Write', 'WORD', args)
    writeUint = async (args) => await this.writeSingle('Write', 'UINT', args)
    writeInt = async (args) => await this.writeSingle('Write', 'INT', args)
    writeInt1Dp = async (args) => await this.writeSingle('Write', 'INT1DP', args)
    writeInt2Dp = async (args) => await this.writeSingle('Write', 'INT2DP', args)
    writeDword = async (args) => await this.writeSingle('Write', 'DWORD', args)
    writeUdint = async (args) => await this.writeSingle('Write', 'UDINT', args)
    writeDint = async (args) => await this.writeSingle('Write', 'DINT', args)
    writeReal = async (args) => await this.writeSingle('Write', 'REAL', args)
    writeLreal = async (args) => await this.writeSingle('Write', 'LREAL', args)
    writeString = async (args) => await this.writeSingle('Write', 'STRING', args)
    writeTime = async (args) => await this.writeSingle('Write', 'TIME', args)
    writeTod = async (args) => await this.writeSingle('Write', 'TOD', args)
    writeDate = async (args) => await this.writeSingle('Write', 'DATE', args)
    writeDt = async (args) => await this.writeSingle('Write', 'DT', args)

    readBool = async (args) => await this.readSingle('Read', 'BOOL', args)
    readByte = async (args) => await this.readSingle('Read', 'BYTE', args)
    readUsint = async (args) => await this.readSingle('Read', 'USINT', args)
    readSint = async (args) => await this.readSingle('Read', 'SINT', args)
    readWord = async (args) => await this.readSingle('Read', 'WORD', args)
    readUint = async (args) => await this.readSingle('Read', 'UINT', args)
    readInt = async (args) => await this.readSingle('Read', 'INT', args)
    readInt1Dp = async (args) => await this.readSingle('Read', 'INT1DP', args)
    readInt2Dp = async (args) => await this.readSingle('Read', 'INT2DP', args)
    readDword = async (args) => await this.readSingle('Read', 'DWORD', args)
    readUdint = async (args) => await this.readSingle('Read', 'UDINT', args)
    readDint = async (args) => await this.readSingle('Read', 'DINT', args)
    readReal = async (args) => await this.readSingle('Read', 'REAL', args)
    readLreal = async (args) => await this.readSingle('Read', 'LREAL', args)
    readString = async (args) => await this.readSingle('Read', 'STRING', args)
    readTime = async (args) => await this.readSingle('Read', 'TIME', args)
    readTod = async (args) => await this.readSingle('Read', 'TOD', args)
    readDate = async (args) => await this.readSingle('Read', 'DATE', args)
    readDt = async (args) => await this.readSingle('Read', 'DT', args)

    writeStruct = async (args) => await this.writeAccessStruct('Write', args)
    readStruct = async (args) => await this.readAccessStruct('Read', args)

    writeArrayOfBool = (args) => this.createArrayDescriptor('Write', 'BOOL', args)
    writeArrayOfByte = (args) => this.createArrayDescriptor('Write', 'BYTE', args)
    writeArrayOfUsint = (args) => this.createArrayDescriptor('Write', 'USINT', args)
    writeArrayOfSint = (args) => this.createArrayDescriptor('Write', 'SINT', args)
    writeArrayOfWord = (args) => this.createArrayDescriptor('Write', 'WORD', args)
    writeArrayOfUint = (args) => this.createArrayDescriptor('Write', 'UINT', args)
    writeArrayOfInt = (args) => this.createArrayDescriptor('Write', 'INT', args)
    writeArrayOfInt1Dp = (args) => this.createArrayDescriptor('Write', 'INT1DP', args)
    writeArrayOfInt2Dp = (args) => this.createArrayDescriptor('Write', 'INT2DP', args)
    writeArrayOfDword = (args) => this.createArrayDescriptor('Write', 'DWORD', args)
    writeArrayOfUdint = (args) => this.createArrayDescriptor('Write', 'UDINT', args)
    writeArrayOfDint = (args) => this.createArrayDescriptor('Write', 'DINT', args)
    writeArrayOfReal = (args) => this.createArrayDescriptor('Write', 'REAL', args)
    writeArrayOfLreal = (args) => this.createArrayDescriptor('Write', 'LREAL', args)
    writeArrayOfString = (args) => this.createArrayDescriptor('Write', 'STRING', args)
    writeArrayOfTime = (args) => this.createArrayDescriptor('Write', 'TIME', args)
    writeArrayOfTod = (args) => this.createArrayDescriptor('Write', 'TOD', args)
    writeArrayOfDate = (args) => this.createArrayDescriptor('Write', 'DATE', args)
    writeArrayOfDt = (args) => this.createArrayDescriptor('Write', 'DT', args)
    writeArrayOfStruct = (args) => this.createArrayDescriptor('Write', 'STRUCT', args)

    readArrayOfBool = (args) => this.createArrayDescriptor('Read', 'BOOL', args)
    readArrayOfByte = (args) => this.createArrayDescriptor('Read', 'BYTE', args)
    readArrayOfUsint = (args) => this.createArrayDescriptor('Read', 'USINT', args)
    readArrayOfSint = (args) => this.createArrayDescriptor('Read', 'SINT', args)
    readArrayOfWord = (args) => this.createArrayDescriptor('Read', 'WORD', args)
    readArrayOfUint = (args) => this.createArrayDescriptor('Read', 'UINT', args)
    readArrayOfInt = (args) => this.createArrayDescriptor('Read', 'INT', args)
    readArrayOfInt1Dp = (args) => this.createArrayDescriptor('Read', 'INT1DP', args)
    readArrayOfInt2Dp = (args) => this.createArrayDescriptor('Read', 'INT2DP', args)
    readArrayOfDword = (args) => this.createArrayDescriptor('Read', 'DWORD', args)
    readArrayOfUdint = (args) => this.createArrayDescriptor('Read', 'UDINT', args)
    readArrayOfDint = (args) => this.createArrayDescriptor('Read', 'DINT', args)
    readArrayOfReal = (args) => this.createArrayDescriptor('Read', 'REAL', args)
    readArrayOfLreal = (args) => this.createArrayDescriptor('Read', 'LREAL', args)
    readArrayOfString = (args) => this.createArrayDescriptor('Read', 'STRING', args)
    readArrayOfTime = (args) => this.createArrayDescriptor('Read', 'TIME', args)
    readArrayOfTod = (args) => this.createArrayDescriptor('Read', 'TOD', args)
    readArrayOfDate = (args) => this.createArrayDescriptor('Read', 'DATE', args)
    readArrayOfDt = (args) => this.createArrayDescriptor('Read', 'DT', args)
    readArrayOfStruct = (args) => this.createArrayDescriptor('Read', 'STRUCT', args)
    ...
    async getUploadInfo() {
        ...
        await this.adsReqSendAsync(adsReq)
    }
     ...
     */
    parseUploadInfo(adsReq, response) {
        var dataString, dataSubString, data, adsReq2;
    ...
    */
    async getConfigFile() {
        ...
        var symbolArray = [],
            name, allSymbols, typeArr, arrayLength, type, elem,
            tcVersion, i;
        ...
        if (this.service.serviceUser || this.service.servicePassword)
            options.headers.push(['Authorization', 'Basic ' + btoa(this.service.serviceUser + ":" + this.service.servicePassword)])
        let response = await fetch(this.service.configFileUrl, options);
        let str = await response.text()
        const configFile = new window.DOMParser().parseFromString(str, "text/xml") as XMLDocument
